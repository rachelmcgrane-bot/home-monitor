...
async def analyse_frame(image_bytes: bytes, enrolled_persons: list[dict]) -> dict:
    ...
    prompt = (
        ...
        '  "activity_type": MUST be exactly one of these THREE values:\n\n'
        '    "cleaning"  — The person is actively doing a job/chore: wiping/scrubbing a surface,\n'
        '                  vacuuming, mopping, sweeping, washing dishes, loading/unloading the\n'
        '                  dishwasher, folding/putting away laundry, tidying by picking things up,\n'
        '                  cooking / preparing food (chopping, stirring, using hob/oven),\n'
        '                  taking out bins, or any other active household task.\n'
        '                  ONLY use if you can see clear active physical effort — NOT standing still,\n'
        '                  NOT sitting, NOT walking through a room.\n\n'
        '    "family"    — The person is physically and actively engaged WITH the children:\n'
        '                  feeding a child, helping with homework, getting a child dressed/ready,\n'
        '                  playing a game WITH a child, reading to a child, cleaning up after a child.\n'
        '                  The adult must be clearly interacting with the child.\n'
        '                  Sitting near children who are playing independently = "present".\n\n'
        '    "present"   — Everything else: the person is detected but is not clearly doing a job\n'
        '                  or actively engaging with a child. Use for: sitting/lying on sofa, watching\n'
        '                  TV, on phone, eating, resting, chatting with another adult, walking through\n'
        '                  a room, working at a desk, exercising, or any situation where you cannot\n'
        '                  confidently classify as "cleaning" or "family".\n'
        '                  When in doubt → "present".\n\n'
        '  RULES:\n'
        '    1. Sitting or lying anywhere → "present". NEVER "cleaning".\n'
        '    2. Only use "cleaning" if you can see active physical effort (tool in hand, visible action).\n'
        '    3. Only use "family" if the adult is clearly engaged WITH a child (not just nearby).\n'
        '    4. Default to "present" whenever you are not confident about "cleaning" or "family".\n\n'
    ...
    try:
        ...
        valid_types = {"cleaning", "family", "present"}
        if result.get("activity_type") not in valid_types:
            result["activity_type"] = "present"
